...
exports.AccessModulesService = void 0;
...
let AccessModulesService = exports.AccessModulesService = class AccessModulesService {
    constructor(paywallModuleRpo, rolePaywallModuleRpo, paywallModuleAction, paywallModuleActionRelation, rolePaywallModuleActionRelation, entityManager) {
        this.paywallModuleRpo = paywallModuleRpo;
        this.rolePaywallModuleRpo = rolePaywallModuleRpo;
        this.paywallModuleAction = paywallModuleAction;
        this.paywallModuleActionRelation = paywallModuleActionRelation;
        this.rolePaywallModuleActionRelation = rolePaywallModuleActionRelation;
    ...
    update(id, updateAccessModule) {
        return `This action updates a #${id} accessModule`;
    }
    async getModulesStatusByRole(roleId) {
        try {
            const paywallModules = await this.paywallModuleRpo.find();
            const paywallModulesWithStatus = await Promise.all(paywallModules.map(async (module) => {
                ...
                return {
                    ...module,
                    idRole: roleId,
                    isActive: rolePaywallModule == null
                        ? false
    ...
    async getPaywallModuleAction(moduleId, role) {
        try {
    ...
    async getModuleActionsWithRelations(moduleId, role) {
        const queryBuilder = this.entityManager
            ...
            .from(paywal_module_actions_entity_1.PaywallModuleAction, 'a')
            .innerJoin(paywall_module_actions_relations_entity_1.PaywallModuleActionRelation, 'b', 'a.id = b."paywallModuleActionId"')
            .innerJoin(paywallModule_entity_1.PaywallModule, 'c', 'c.id = b."paywallModuleId"')
            .leftJoinAndSelect((subQuery) => subQuery
            .from(role_paywall_module_actions_entity_1.RolePaywallModuleActionRelation, 'd')
            .select([
            'd."role"',
            'd."paywallModuleId"',
            'd."paywallModuleActionRelationId"',
        ]), 'd', 'd."paywallModuleId" = c.id'
            + ' AND d."paywallModuleActionRelationId" = b.id'
            + ' AND d."role" = :role', { role: role })
            .where('c.id = :moduleId', { moduleId: moduleId });
        return queryBuilder.getRawMany();
    }
    async getUpdatePaywallModuleAction(moduleId, role, isActiveP) {
        ...
        try {
            ...
            if ((await resultModule).length > 0) {
                ...
                var updateModule = this.entityManager
                    .createQueryBuilder()
                    .update(role_paywall_module_entity_1.RolePaywallModule)
                    .set(params)
                ...
                if (!paywallModule) {
                    console.log(...oo_oo(`1819871408_166_10_166_74_4`, 'No se encontró el módulo con el ID proporcionado.'));
                    return;
                }
                var paramsInsert = {
    ...
    async setUpdateRolModuleAction(moduleId, ActionRelationId, role) {
        const moduleIdAsNumber = parseInt(moduleId, 10);
        try {
            var resultModule = this.getQueryRolModuleAction(moduleId, ActionRelationId, role);
            if ((await resultModule).length > 0) {
                var updateModule = this.entityManager
                    .createQueryBuilder()
                    .delete()
                    .from(role_paywall_module_actions_entity_1.RolePaywallModuleActionRelation)
                    .where('role = :role', { role: role })
                    .andWhere('paywallModuleId = :moduleId', { moduleId: moduleId })
                ...
                console.log(...oo_oo(`1819871408_229_8_229_45_4`, ' REGISTRANDO AQUI ... '));
                const moduleIdAsNumber = parseInt(moduleId, 10);
                const paywallModule = await this.paywallModuleRpo.findOne({
                    where: { id: moduleIdAsNumber },
                });
                if (!paywallModule) {
                    console.log(...oo_oo(`1819871408_237_10_237_74_4`, 'No se encontró el módulo con el ID proporcionado.'));
                    return;
                }
                const ActionRelationIdAsNumber = parseInt(ActionRelationId, 10);
                const paywallModuleActionRelation = await this.paywallModuleActionRelation.findOne({
                    where: { id: ActionRelationIdAsNumber },
                });
                if (!paywallModuleActionRelation) {
                    console.log(...oo_oo(`1819871408_249_10_249_74_4`, 'No se encontró el módulo con el ID proporcionado.'));
                    return;
                }
                var paramsInsert = {
                ...
                if ((await this.rolePaywallModuleActionRelation.insert(paramsInsert))
                    .raw > 0) {
                    return this.getQueryRolModuleAction(moduleId, ActionRelationId, role);
                }
                else {
    ...
    async getQueryRolModule(moduleId, role) {
        const queryBuilder = this.entityManager
            .createQueryBuilder()
            .select(['a.*'])
            .from(role_paywall_module_entity_1.RolePaywallModule, 'a')
            .where('a.role = :role', { role: role })
            .andWhere('a.paywallModuleId = :moduleId', { moduleId: moduleId });
        return queryBuilder.getRawMany();
    }
    async getQueryRolModuleAction(moduleId, ActionRelationId, role) {
        const queryBuilder = this.entityManager.createQueryBuilder()
            .select(['a.*'])
            .from(role_paywall_module_actions_entity_1.RolePaywallModuleActionRelation, 'a')
            .where('a.role = :role', { role: role })
            .andWhere('a.paywallModuleId = :moduleId', { moduleId: moduleId })
            .andWhere('a.paywallModuleActionRelationId = :ActionRelationId', { ActionRelationId: ActionRelationId });
        return queryBuilder.getRawMany();
    }
    async getRolePaywallModulle(roleId) {
        console.log(...oo_oo(`1819871408_313_4_313_38_4`, 'ingresó al servicio'));
        try {
    ...
    async getRolePaywallModule(roleId, body) {
        try {
    ...
    async getActiveModulesByRole(roleId) {
        return this.paywallModuleRpo
            ...
            .innerJoin(role_paywall_module_entity_1.RolePaywallModule, 'b', 'a.id = b.paywallModuleId')
            .where('b.role = :roleId AND b.isActive = :isActive', { roleId, isActive: true })
            .getRawMany();
    }
};
exports.AccessModulesService = AccessModulesService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(paywallModule_entity_1.PaywallModule)),
    __param(1, (0, typeorm_1.InjectRepository)(role_paywall_module_entity_1.RolePaywallModule)),
    __param(2, (0, typeorm_1.InjectRepository)(paywal_module_actions_entity_1.PaywallModuleAction)),
    __param(3, (0, typeorm_1.InjectRepository)(paywall_module_actions_relations_entity_1.PaywallModuleActionRelation)),
    __param(4, (0, typeorm_1.InjectRepository)(role_paywall_module_actions_entity_1.RolePaywallModuleActionRelation)),
...
;
oo_te;
//# sourceMappingURL=access-modules.service.js.map